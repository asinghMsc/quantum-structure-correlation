...
\textbf{Definition.} Structure refers to non-random correlations between subsystems, quantified by mutual information that exceeds a randomised baseline. Formally, given a bipartition of a system into subsystems $A$ and $B$, structure is present when $I(A;B) > I(A;B)_{\text{random}}$, where $I(A;B)$ denotes the mutual information between $A$ and $B$, and $I(A;B)_{\text{random}}$ denotes the expected mutual information under a randomised ensemble.
...
\textbf{Measurement.} Preservation is quantified by the correlation retention ratio:
\begin{equation}
...
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{Structured Circuits} & \textbf{Randomised Circuits} & \textbf{Difference} \\
\midrule
$I(A;B)$ post-interference & $8.00 \pm 0.00$ bits & $4.22 \pm 1.08$ bits & $+3.78$ bits \\
Correlation Retention $R$ & $1.00 \pm 0.00$ & $1.00 \pm 0.00$ & $0.00$ \\
State Fidelity $F$ & $0.008 \pm 0.022$ & $0.023 \pm 0.044$ & $-0.015$ \\
z-score ($I_{\text{post}}$) & --- & --- & $\mathbf{+3.50}$ \\
\bottomrule
\end{tabular}
\caption{Summary statistics for structured and randomised circuits under the interference protocol.}
\label{tab:summary}
\end{table}

\subsection{Primary Finding: Differential Mutual Information}
...
\begin{itemize}
    \item \textbf{Structured circuits:} $I(A;B) = 8.00$ bits (maximum possible for 4 Bell pairs)
    \item \textbf{Randomised circuits:} $I(A;B) = 4.22 \pm 1.08$ bits (variable, depending on random entanglement patterns)
    \item \textbf{z-score = 3.50}, corresponding to $p < 0.001$ (two-tailed)
\end{itemize}
